...
Ext.define('CustomApp', {
    ...
    processDefectsPerInterval:function(defectsPerInterval, counter, event){
        ...
        _.each(defectsPerInterval, function(defect){
            ...
            chunksOfTime.push(parseFloat(this.getDiffInDays(defect[start], defect[end])));
        },this);
    ...
    getLowerGroup:function(arr){
        var half = Math.floor(arr.length/2);
        var lGroup = [];
        if (arr.length % 2) { //odd, median will be included in both halves
            lGroup = arr.slice(0, half+1); //slice extracts up to but not including end
        }
        else{//even, first middle value will be included in the first half
            lGroup = arr.slice(0, half); //slice extracts from 0 to including arr[half-1], not including arr[half]
        }
        return lGroup;
    },
    
    getUpperGroup:function(arr){
        var half = Math.floor(arr.length/2);
        var uGroup = [];
        if (arr.length % 2) { //odd, median will be included in both halves
            uGroup = arr.slice(half); //slice extracts to the end of the array
        }
        else{//even, second middle value will be included in the second half
            uGroup = arr.slice(half); 
        }
        return uGroup;
    },
    ...
    makeGrid:function(){
        ...
        this.add({
            ...
            columnCfgs: [
                ...
                {text: 'Time Between Events: Lower Quartile',  dataIndex: 'lowerQuartile'}
            ]
        });
